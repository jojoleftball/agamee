...
export default function PlantingGround() {
  ...
  const [plantSlots] = useState<PlantSlot[]>([
  ...
  const handleDragStart = (itemId: string) => {
    ...
    if (itemData && (itemData.category === 'flower' || itemData.category === 'vegetable' || itemData.category === 'tree')) {
  ...
  const handleDrop = (slotId: string) => {
    if (draggedItemId) {
      ...
      if (slot && !slot.plantedItem) {
        slot.plantedItem = draggedItemId;
  ...
  const handleWater = (slotId: string) => {
    const slot = plantSlots.find(s => s.id === slotId);
    if (slot && slot.plantedItem && slot.waterLevel < 100) {
      slot.waterLevel = Math.min(100, slot.waterLevel + 25);
      slot.lastWatered = Date.now();
      
      if (slot.waterLevel >= 100) {
    ...
    <div className="w-full h-full flex items-center justify-center p-4">
      ...
      >
        <div className="absolute top-4 left-4 bg-white/95 backdrop-blur-sm px-4 py-2 rounded-xl shadow-lg border-2 border-green-600">
          ...
          <p className="text-xs text-gray-600 mt-1">Drag plants here to grow them</p>
        </div>
              ...
              {slot.plantedItem ? (
                <>
                  ...
                  <button
                    ...
                    className="mt-2 bg-blue-500 hover:bg-blue-600 disabled:bg-gray-400 text-white p-2 rounded-lg transition-all active:scale-95"
                  >
                  ...
                  {slot.growthStage >= 100 && (
                    <button
                      ...
                      className="absolute -top-2 -right-2 bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg transition-all active:scale-95"
                    >
                      Harvest
                    </button>
                  )}
                ...
                <div className="text-center">
                  <Sprout className="w-8 h-8 text-amber-700 mx-auto mb-1 opacity-50" />
